...
class WidgetSchema:
    ...
    def __foundation_model_id_selection(
        ...
        if self.model_family == "bedrock":
            ...
            model_id_small = env("OPENAI_SMALL_MODEL")
...
class LLMTagsPrediction:
    ...
    def __predict_entry(self, excerpt: str):
        ...
        def select_model_instance(model_name: str):
            if self.model_family == "bedrock":
                model = ChatBedrockConverse(model_id=model_name, temperature=0)
            elif self.model_family == "openai":
        ...
        for k, v in prediction.items():
            ...
            if type_ == self.widgets.mappings_instance.WidgetTypes.matrix1dWidget:
                ...
                for c in v:
                    ...
                    if pillar not in results[k].keys():
                        results[k].update({pillar: {}})
                ...
                for c in v:
                    ...
                    if len(elements) == 3:  # prediction without subcolumn
                        ...
                        if pillar not in results[k].keys():
                            results[k].update({pillar: {}})
                        if subpillar not in results[k][pillar].keys():
                            results[k][pillar].update({subpillar: {}})
