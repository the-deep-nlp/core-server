...
class ModelPerformance:
    ...
    def project_wise_perf_metrics(
        ...
        for category in self.categories:
            for project_grp_gt, project_grp_pred in zip(
                ...
                list(
                    self.dataframe.groupby("project_id")[f"{category}_pred_transformed"]
                ),
            ):
    ...
    def per_tag_perf_metrics(self) -> pd.DataFrame:
        ...
        per_tag_precision_df = pd.DataFrame.from_dict(
            tag_precision_perf_metrics, orient="index", columns=["precision"]
        )
        per_tag_recall_df = pd.DataFrame.from_dict(
            tag_recall_perf_metrics, orient="index", columns=["recall"]
        )
        per_tag_f1score_df = pd.DataFrame.from_dict(
            tag_f1score_perf_metrics, orient="index", columns=["f1score"]
        )
        per_tag_support_df = pd.DataFrame.from_dict(
            tag_support_perf_metrics, orient="index", columns=["support"]
        )
    ...
    def calculate_ratios(self) -> pd.DataFrame:
        ...
        for category in self.categories:
            ratios_df[f"{category}_completely_matched"] = pd.Series(
                list(
                    map(
                        self.completely_matched_tags,
                        list(
                            zip(
                                np.array(self.dataframe[f"{category}_transformed"]),
                                np.array(
            ...
            ratios_df[f"{category}_completely_matched"] /= len(
                cat_to_mlb[category].classes_
            )

            ratios_df[f"{category}_missing"] = pd.Series(
                list(
                    map(
                        self.missing_tags,
                        list(
            ...
            ratios_df[f"{category}_wrong"] = pd.Series(
                list(
                    map(
                        self.wrong_tags,
                        list(
        ...
        ratios_df = pd.concat(
            [ratios_df, self.dataframe[["entry_id", "project_id"]]], axis=1
        )
    ...
    def per_project_calc_ratios(self) -> pd.DataFrame:
        ...
        final_df = pd.DataFrame()
        for category in self.categories:
            final_df[f"{category}_completely_matched_mean"] = ratios_df.groupby(
                ["project_id"]
            )[f"{category}_completely_matched"].mean()
            final_df[f"{category}_missing_mean"] = ratios_df.groupby(["project_id"])[
                f"{category}_missing"
            ].mean()
            final_df[f"{category}_wrong_mean"] = ratios_df.groupby(["project_id"])[
                f"{category}_wrong"
            ].mean()
...
if __name__ == "__main__":
    df = (
        pd.read_csv("csvfiles/sampled_data_with_predictions_testset.csv")
        .sample(n=50, random_state=1234)
        .reset_index()
    )
