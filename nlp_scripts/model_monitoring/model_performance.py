...
class ModelPerformance:
        ...
        for category in categories:
            if category in self.dataframe.columns:
                self.dataframe = self.dataframe.with_columns(
                    pl.col(category).apply(literal_eval).cast(pl.List(pl.Utf8)).alias(category)
                )
            category_pred = f"{category}_pred"
            if category_pred in self.dataframe.columns:
                self.dataframe = self.dataframe.with_columns(
                    pl.col(category_pred).apply(literal_eval).cast(pl.List(pl.Utf8)).alias(category_pred)
                )
    ...
    def _create_mlb_mappings(self):
        ...
        self.mlb_specific_needs_groups.fit_transform([[s] for s in specific_needs_groups_lst])
    ...
    def label_transform(self):
        ...
        for category in categories:
            if category in self.dataframe.columns:
                ...
                self.dataframe = self.dataframe.with_columns(
                    pl.col(category).apply(lambda x: cat_to_mlb[category].transform([x])[0]).alias(cat_name)
                )
            if f"{category}_pred" in self.dataframe.columns:
                cat_name = f"{category}_pred_transformed"
                self.dataframe = self.dataframe.with_columns(
                    pl.col(f"{category}_pred").apply(lambda x: cat_to_mlb[category].transform([x])[0]).alias(cat_name)
                )

    def projectwise_perf_metrics(self, metrics_average_type: str="macro") -> pl.DataFrame:
        """
        ...
        for category in categories:
            if category in self.dataframe.columns:
                for project_grp in self.dataframe.groupby("project_id", maintain_order=True):
                    # Note that
                    # project_grp[0] -> project_id
                    # project_grp[1] -> dataframe containing remaining data
                    precision, recall, f1score, _ = precision_recall_fscore_support(
                        y_true=project_grp[1][f"{category}_transformed"].apply(list).to_list(),
                        y_pred=project_grp[1][f"{category}_pred_transformed"].apply(list).to_list(),
    ...
    def overall_projects_perf_metrics(self, metrics_average_type: str="macro") -> pl.DataFrame:
        """
        ...
        for category in categories:
            if category in self.dataframe.columns:
                precision, recall, f1score, _ = precision_recall_fscore_support(
                    y_true=self.dataframe[f"{category}_transformed"].apply(list).to_list(),
                    y_pred=self.dataframe[f"{category}_pred_transformed"].apply(list).to_list(),
                    average=metrics_average_type,
                    zero_division=0
                )
    ...
    def per_tag_perf_metrics(self) -> pl.DataFrame:
        ...
        """
        def generate_df_with_extra_cols(tags_scores_dict: dict, metrics_type: str) -> pl.DataFrame:
            df_temp = pl.DataFrame(tags_scores_dict).transpose(include_header=True, header_name="tags", column_names=["scores"])
        ...
        precision_df = generate_df_with_extra_cols(tag_precision_perf_metrics, "sectors")
    ...
    def _tags_matching_ratios(self, category, gt_embed: list, pred_embed: list) -> pl.DataFrame:
        """
        Calculates the ratios based on the encoding values of the tags
        """
        return {
            f"completely_matched_{category}": sum([e1==e2 for e1, e2 in zip(gt_embed, pred_embed)]) / len(gt_embed),
            f"missing_{category}": sum([e1>e2 for e1, e2 in zip(gt_embed, pred_embed)]) / len(gt_embed),
            f"wrong_{category}": sum([e1<e2 for e1, e2 in zip(gt_embed, pred_embed)]) / len(gt_embed)
        }
    ...
    def calculate_ratios(self) -> pl.DataFrame:
        ...
        for category in categories:
            if category in self.dataframe.columns:
                ...
                final_df = pl.concat([final_df, pl.DataFrame(temp_lst)], how="horizontal")
        ...
        for project_grp in ratios_df.groupby("project_id", maintain_order=True):
            ...
            for category in categories:
                if f"completely_matched_{category}" in project_grp_df.columns:
                    temp_df = temp_df.with_columns(pl.lit(project_grp_df[f"completely_matched_{category}"].mean()).alias(f"completely_matched_{category}_mean"))
                if f"missing_{category}" in project_grp_df.columns:
                    temp_df = temp_df.with_columns(pl.lit(project_grp_df[f"missing_{category}"].mean()).alias(f"missing_{category}_mean"))
                if f"wrong_{category}" in project_grp_df.columns:
                    temp_df = temp_df.with_columns(pl.lit(project_grp_df[f"wrong_{category}"].mean()).alias(f"wrong_{category}_mean"))
...
if __name__ == "__main__":
    ...
    pl_df = pl.DataFrame({
        ...
        "sectors": [json.dumps(["first_level_tags->sectors->Agriculture", "first_level_tags->sectors->Shelter"]), json.dumps(["first_level_tags->sectors->Agriculture"])],
        "pillars_1d": [json.dumps(["first_level_tags->pillars_1d->Casualties"]), json.dumps(["first_level_tags->pillars_1d->Context"])],
        "sectors_pred": [json.dumps(["first_level_tags->sectors->Agriculture", "first_level_tags->sectors->Protection"]), json.dumps(["first_level_tags->sectors->Agriculture"])],
        "pillars_1d_pred": [json.dumps(["first_level_tags->pillars_1d->Casualties"]), json.dumps(["first_level_tags->pillars_1d->Context"])]
    })
